...
impl Sandbox {
    ...
    pub fn resolve_path(&self, device_id: &str, original_name: &str) -> PathBuf {
        let device_dir = self.root.join(device_id);
    ...
    pub fn path_for_incoming(&self, device_id: &str, original_file_name: &str) -> PathBuf {
        ...
        let safe_name = sanitize_segment(std::path::Path::new(original_file_name).file_name()
            .and_then(|n| n.to_str())
            .unwrap_or("unnamed"));
    ...
    pub fn prepare_incoming_resume(
        ...
        if let Some(existing) = self.read_incoming_resume_meta(file_id) {
            if is_matching_incoming_resume(&existing, device_id, device_name, trust_fingerprint, trust_public_key, file_name, file_size, sha256) {
    ...
    pub fn complete_incoming_resume(&self, file_id: &str) -> Result<PathBuf, String> {
        let meta = self.read_incoming_resume_meta(file_id)
    ...
    fn resume_meta_path(&self, file_id: &str) -> PathBuf {
        let resume_dir = self.root.join(".resume");
